    ...
    <div>
       <header className='bg-[#F8F9FA] w-full h-20 sticky z-50 top-0'>
        <nav className='flex justify-between items-center w-full mx-auto h-full px-10 py-2 max-md:px-4'>
            <h1 className='font-semibold text-lg max-md:text-xs '>AI-Based Dermatological <br></br> Diagnosis Tool</h1>
            <div className='flex gap-10 justify-center items-center'>
                <div className='flex gap-4 max-md:gap-2'>
                    <Link to='Home' spy={true} smooth={true} offset={-100} duration={500}>
                        <p className='font-medium max-md:text-xs px-4 py-1 rounded-xl shadow-md bg-[#fffff] text-[#000] hover:transition all 0.2s ease-in-out cursor-pointer hover:bg-lime-100 '>Home</p>
                    </Link>
                    <Link to='About' spy={true} smooth={true} offset={-70} duration={500}>
                      <p className='font-medium max-md:text-xs px-4 py-1 rounded-xl shadow-md bg-[#fffff] text-[#000] hover:transition all 0.2s ease-in-out cursor-pointer hover:bg-lime-100'>About</p>
                    </Link>
                    <Link to='Team' spy={true} smooth={true} offset={-70} duration={500}>
                    <p className='font-medium max-md:text-xs px-4 py-1 rounded-xl shadow-md bg-[#fffff] text-[#000] hover:transition all 0.2s ease-in-out cursor-pointer hover:bg-lime-100'>Team</p>
                    </Link>
                </div>
      ...
      <div className='h-10 my-4 flex justify-end items-center max-md:justify-between max-md:my-8'>
            <a href="https://www.srmist.edu.in/" target="__blank">
              <div>
                <img className="w-36 bg-transparent h-auto max-md:hidden" src={SRMLogo} alt="srmlogo" />
              </div>
            </a>
              ...
              <div className='flex gap-4 items-center w-fit mr-10 max-md:w-auto max-md:text-sm max-md:m-0 max-md:mr-4'>
                  <div onClick={changeModalState} className='bg-lime-100 hover:bg-lime-200 transition-all 0.2s ease-in-out cursor-pointer btn flex justify-center items-center px-6 py-2 text-[#000] rounded-full'>Login with Mobile Number</div>
              </div>
